...
/**
 * A record is a file, not a container: `{acc}-{slug}.mdx` directly under
 * `RECORDS_BASE`. (It briefly held one `index.mdx` per directory instead —
 * abandoned once every directory turned out to hold exactly one file and the
 * nesting had nothing to justify it.)
 *
 ...
 *
 *   records   2026.04-rochester-map.mdx
 */
const recordId = (entry: string) => entry.replace(/\.mdx$/i, '');

/** The leading `YYYY.NN` of a record filename, or null if absent. */
const accFromFilename = (filename: string): string | null =>
	ACCESSION.test(filename.slice(0, 7)) ? filename.slice(0, 7) : null;

/**
 * Records: accession numbers are unique across the register, and the filename
 * states the same accession as the frontmatter.
 *
 ...
 * register's history; a filename disagreeing with its own frontmatter means
	...
	return {
		...
		load: async (context: LoaderContext) => {
			...
			for (const entry of context.store.values()) {
				...
				const filenameAcc = accFromFilename(entry.id);
				if (filenameAcc === null) {
					mismatched.push(
						`  "${entry.id}" — filename must begin with an accession number, e.g. "${acc}-some-slug.mdx"`,
					);
				} else if (filenameAcc !== acc) {
					mismatched.push(
						`  "${entry.id}" — filename says ${filenameAcc}, frontmatter says ${acc}`,
					);
				}
			...
			if (mismatched.length > 0) {
				throw new Error(
					`Record filename does not match its accession number:\n${mismatched.join('\n')}\n\n` +
						`A record file is named {acc}-{slug}.mdx. The filename and the frontmatter ` +
						`are one source of truth, checked rather than trusted — rename the file ` +
